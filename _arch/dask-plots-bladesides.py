...
for file in filelist:
    ...
    ax0.set_title(str('Sound pressure at sside. Time: ' + str(timestep) + ' [Pa]'))
    spldb_plot = ax1.scatter(z, x, c=spl_db, s=0.5, vmin=0, vmax=200, cmap=plt.cm.bone)
    pressure.colorbar(spldb_plot, ax=ax1)
    ax1.set_title(str('SPLdB at sside. Time: ' + str(timestep) + ' [dB]'))
    os.chdir(path_plots)
    plt.savefig(str('sside_spl_t_' + str(timestep) + '.png'))
    plt.close()
'''
