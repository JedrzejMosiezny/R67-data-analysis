...
for file in filelist
    ...
    y = acu['y-coordinate']
    z = acu['z-coordinate']
    spl = pstatic['sound-pressure']
    dbl = pstatic['db-level']
    fig, (ax0, ax1) = plt.subplots(nrows=2, figsize=(20, 20), dpi=300)
    spl_plot = ax0.tripcolor(z, x, spl, cmap=plt.cm.bone, vmin=-10000, vmax=10000)
    fig.colorbar(spl_plot, ax=ax0)
    ax0.set_title(str('Sound pressure. Int-01. Time: ' str(timestep)))
    dbl_plot = ax1.tripcolor(z, x, dbl, cmap=plt.cm.jet, vmin=0, vmax=194)
